...
1. **Resume** — replace `public/resume.pdf` with your real PDF (keep the filename, or update `resumeUrl` in `data/profile.ts`). The current file is a placeholder.
2. **Social links** — swap the placeholder GitHub and LinkedIn URLs in `data/profile.ts`.
3. **Contact form** — it currently validates on the client and simulates a send; nothing is emailed yet. Open `components/Contact.tsx` and find the `WIRE UP YOUR BACKEND HERE` block inside `handleSubmit`. It has ready-to-paste snippets for Formspree or your own `app/api/contact/route.ts` route handler. Re-validate on the server if you add a route.
